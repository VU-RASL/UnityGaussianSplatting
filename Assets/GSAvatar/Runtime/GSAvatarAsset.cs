...
{
    public class GSAvatarAsset : ScriptableObject
    {
        ...
        // GPU Buffers
        public ComputeBuffer m_GpuFaces;
        public ComputeBuffer m_GpuGaussiansToFace;
        public ComputeBuffer m_GpuOffsets;
        // public ComputeBuffer m_GpuScaling;
        ...
        public GSAvatarAsset(HahaAvatarData data)
        {
        ...
        // public Texture m_GpuColorData; 
            ...
            m_DataHash = new Hash128((uint)m_SplatCount,(uint)m_FormatVersion,0,0);

            m_GpuOffsets = new ComputeBuffer(data.offsets.Length,sizeof(float) * 3);
            m_GpuOffsets.SetData(data.offsets);
            
            m_GpuFaces = new ComputeBuffer(data.facesToVerts.Length, sizeof(int) * 3);
            m_GpuFaces.SetData(data.facesToVerts);

            m_GpuGaussiansToFace = new ComputeBuffer(data.gaussianToFace.Length, sizeof(int));
            m_GpuGaussiansToFace.SetData(data.gaussianToFace);

            m_GpuPosData = new GraphicsBuffer(GraphicsBuffer.Target.Raw | GraphicsBuffer.Target.CopySource, (int) (data.splatCount * 3), 4) { name = "GaussianPosData" }; 
            uint[] initData = new uint[data.splatCount * 3];
            m_GpuPosData.SetData(initData);



            // Create the m_GpuOtherData buffer
            int otherDataStride = sizeof(float) * 8; // 4 (rotation) + 3 (scaling) + 1 (opacity)
            m_GpuOtherData = new GraphicsBuffer(GraphicsBuffer.Target.Structured, data.rotations.Length, otherDataStride);
            float[] otherData = new float[data.rotations.Length * 8];
            for (int i = 0; i < data.rotations.Length; i++)
            {
                otherData[i * 8 + 0] = data.rotations[i].x;
                otherData[i * 8 + 1] = data.rotations[i].y;
                otherData[i * 8 + 2] = data.rotations[i].z;
                otherData[i * 8 + 3] = data.rotations[i].w;
                otherData[i * 8 + 4] = data.scaling[i].x;
                otherData[i * 8 + 5] = data.scaling[i].y;
                otherData[i * 8 + 6] = data.scaling[i].z;
                otherData[i * 8 + 7] = data.opacities[i];
            }
            m_GpuOtherData.SetData(otherData);

            // Create the m_GpuSHData buffer
            int shDataStride = sizeof(float) * 3; // Each color is a float3
            m_GpuSHData = new GraphicsBuffer(GraphicsBuffer.Target.Structured, data.colors.Length, shDataStride);
            float[] shData = new float[data.colors.Length * 3];
            for (int i = 0; i < data.colors.Length; i++)
            {
                shData[i * 3 + 0] = data.colors[i].x;
                shData[i * 3 + 1] = data.colors[i].y;
                shData[i * 3 + 2] = data.colors[i].z;
            }
